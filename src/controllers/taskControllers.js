...
export const getAllTasks = TryCatch(async(req, res, next)=>{
    ...
    if(!tasks || tasks.length) return next(new Errorhandler('No tasks found', 404));
...
export const createTask = TryCatch(async(req, res, next)=>{
    ...
    if(!title || !description || !status || !dueDate) return next(new Errorhandler('Please fill all the fields', 400))
...
export const updateTask = TryCatch(async(req, res, next)=>{
    ...
    if(!updates) return next(new Errorhandler('Nothing found to be updated', 400));
